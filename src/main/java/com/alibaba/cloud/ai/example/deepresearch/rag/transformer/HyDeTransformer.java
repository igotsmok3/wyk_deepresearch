 ...
 *
 * <p>
 * 项目职责：RAG 查询前处理阶段的增强组件，实现 Spring AI 的 QueryTransformer 接口； 核心思路是让 LLM
 * 针对用户问题生成一段假设性答案，其向量与真实文档更接近，从而提高召回率。 生成失败时降级返回原始查询，保证管道不中断。
 *
 * <p>
 * 被使用情况：在 DefaultHybridRagProcessor 构造时，若 hypotheticalDocumentEmbeddingEnabled=true 则通过
 * HyDeTransformer.builder() 创建实例，在 preProcess 阶段对每条查询执行 HyDE 转换。
 *
 * @author benym
 */
