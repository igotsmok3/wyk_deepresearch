 ...
 *
 * <p>
 * 项目职责：由 {@code SmartAgentSelectionHelperService} 组装并返回， 作为节点执行前的 Agent
 * 决策载体，{@code isSmartAgent} 标识是否启用了多 Agent 路由， {@code stateUpdate} 携带需要写回图状态的配置信息。
 *
 * <p>
 * 被使用情况：{@code ResearcherNode} 调用 {@code selectSmartAgent} 获取该结果， 并据此选择实际执行研究任务的 Agent；
