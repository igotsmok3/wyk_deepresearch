 ...
 *
 * <p>
 * 项目职责：作为 Spring AI Tool 封装
 * {@link com.alibaba.cloud.ai.example.deepresearch.service.SearchFilterService} 的调用，在
 * Researcher Agent 执行搜索时对不可信网站的内容进行过滤， 返回按信任权重排序的 {@code SearchContentWithWeight}
 * 列表，提升研究结果质量。
 *
 * <p>
 * 被使用情况：由 Spring AI 工具调用框架管理，在 ResearcherNode 中按需创建实例并注入 Researcher ChatClient； 不由 Spring
 * 容器直接管理（非 @Service），而是通过节点构造时手动实例化。
 *
 * @author vlsmb
 * @since 2025/7/10
 */
