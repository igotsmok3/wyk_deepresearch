 ...
 *
 * <p>
 * 项目职责：定义搜索结果后处理的核心逻辑——加载站点权重、执行"首尾高权重"排列算法 （将高可信域名排在列表首尾以利用 LLM 注意力偏置）、以及过滤权重为负的黑名单站点。
 * 子类只需实现 {@code loadWebsiteWeight()} 提供权重来源即可复用全部过滤逻辑。
 *
 * <p>
 * 被使用情况：由 {@link LocalConfigSearchFilterService} 继承并注册为 Spring Bean， 被
 * {@code SearchInfoService}、{@code ResearcherNode}、{@code BackgroundInvestigationNode} 和
 * {@code SearchFilterTool} 使用；单元测试中通过匿名子类 {@code TestSearchFilterService} 覆盖。
 *
 * @author vlsmb
 * @since 2025/7/10
 */
public abstract class SearchFilterService {
	 ...
	 * 奇数索引 → 后半段（反转顺序）：[D, B]
	 * 最终拼接：[A, C, E, D, B]
	 * </pre> 效果：权重最高的 A 在最前，第二高的 B 在最后，利用 LLM 对序列首尾的注意力偏置提升权威来源的影响力。
