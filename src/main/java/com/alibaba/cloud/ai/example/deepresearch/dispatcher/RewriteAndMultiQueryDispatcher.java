 ...
 *
 * <p>项目职责：dispatcher 层的边路由实现，读取 {@code RewriteAndMultiQueryNode}（负责对原始查询
 * 进行改写和多路扩展）写入的 {@code rewrite_multi_query_next_node} 键：有网络搜索需求时路由至
 ...
 * 通过 {@code addConditionalEdges("rewrite_multi_query", ...)} 注册到图配置中。
 */
