 ...
 *
 * <p>
 * 项目职责：协调 {@code QuestionClassifierService} 和 {@code SearchPlatformSelectionService}，
 * 根据问题分类结果选择对应专属 ChatClient（学术/生活旅游/百科/数据分析/通用）， 并将选中的 Agent、AgentType、搜索平台列表及搜索策略描述封装为
 * {@code AgentDispatchResult} 返回；任何异常均回退到通用研究 Agent，保证主流程不中断。 仅在 smart-agents 功能开启时注册为
 * Bean。
 *
 * <p>
 * 被使用情况：被 {@code SmartAgentSelectionHelperService} 和 {@code ResearcherNode} 注入，
