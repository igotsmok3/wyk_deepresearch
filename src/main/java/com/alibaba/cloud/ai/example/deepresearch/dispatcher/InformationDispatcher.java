 ...
 *
 * <p>项目职责：dispatcher 层的边路由实现，读取 {@code InformationNode} 写入的
 * {@code information_next_node} 键：计划解析成功时路由至 {@code "human_feedback"}（等待确认）
 * 或 {@code "research_team"}（自动接受）；解析失败时回退至 {@code "planner"} 重规划，
 * 超出重试次数则终止（{@code END}）。
 *
 * <p>被使用情况：由 {@link com.alibaba.cloud.ai.example.deepresearch.config.DeepResearchConfiguration}
 * 通过 {@code addConditionalEdges("information", ...)} 注册到图配置中。
 */
