 ...
 *
 * <p>
 * 项目职责：每个枚举值对应一个专领域 Agent（学术研究、生活旅游、百科、数据分析、通用研究）， 携带该 Agent 的描述、Prompt 文件路径和参考资料格式指引，驱动多
 * Agent 路由策略。
 *
 * <p>
 * 被使用情况：{@code QuestionClassifierService} 根据问题分类返回对应枚举； {@code AgentPromptTemplateUtil}
 * 根据枚举加载对应 Prompt 文件； {@code AgentsConfiguration} 为每个枚举值构建独立的 ChatClient Bean；
