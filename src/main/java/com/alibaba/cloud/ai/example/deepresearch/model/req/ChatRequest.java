 ...
 *
 * <p>
 * 项目职责：作为 {@code POST /chat/stream} 接口的请求体， 携带用户问题、会话 ID、最大迭代次数、搜索引擎选择等控制参数， 经
 * {@code ChatRequestProcess} 填充默认值后传入图执行流程。
 *
 * <p>
 * 被使用情况：{@code ChatController} 接收并处理该请求； {@code ChatRequestProcess} 补全默认值并将字段写入图状态 Map；
