 ...
 *
 * <p>
 * 项目职责：从 OverAllState 的 {@code current_plan} 中找到分配给自己（{@code coder_N}）且待执行的 PROCESSING
 * 步骤，调用 coderAgent 流式生成代码结果。支持 Reflection 机制：步骤完成后可触发质量评估， 评估未通过则重新执行。写入 OverAllState 的
 * {@code coder_content_{N}} 键（流式 Flux）。 使用 MCP 工厂按需注入工具回调。
 *
 * <p>
 * 被使用情况：由 {@code DeepResearchConfiguration#addCoderNodes} 根据配置动态注册多个实例
