 ...
 *
 * <p>
 * 项目职责：属于配置层的核心类，将所有图节点（node/）和边调度器（dispatcher/）串联成完整的有向图。
 ...
 *
 * <p>
 * 被使用情况：由 Spring 容器直接管理；产出的 {@code deepResearch} StateGraph Bean 被 {@code ChatController}
 * 编译为 {@code CompiledGraph} 后驱动整个研究流程执行。 {@code ReflectionProcessor} 的创建也在本类中完成。
 *
 * @author yingzi
