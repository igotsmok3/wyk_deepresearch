 ...
 *
 * <p>
 * 项目职责：图流程中 short_user_role_memory 之后的第一个决策节点。从 OverAllState 读取 {@code query} 和
 * {@code session_id}，按顺序组装：用户角色画像、coordinator 系统提示词、
 * 多轮对话历史（MessageWindowChatMemory）和当前用户提问，然后调用 coordinatorAgent。 LLM
 * 以工具调用方式表达"需要深度研究"的意图。写入 OverAllState：
 * <ul>
 ...
 *
 * <p>
 * 被使用情况：由 {@code DeepResearchConfiguration} 以节点名 {@code coordinator} 注册到图中；
